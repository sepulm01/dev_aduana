...
static GstElement *sgie0 = NULL;
static GstElement *sgie1 = NULL;
...
main (int argc, char *argv[])
{
  ...
  appctx.preprocess = gst_element_factory_make("identity", "preprocess-plugin");
  ...
  /* Secondary GIE 0 (e.g. FaceDetect on PeopleNet crops, or LPDNet on TrafficCamNet) */
  {
    NvDsGieType sgie0_type = NVDS_GIE_PLUGIN_INFER;
    if (nvds_parse_gie_type(&sgie0_type, argv[1], "secondary-gie0") == NVDS_YAML_PARSER_SUCCESS) {
      GstElement *sg = gst_element_factory_make("nvinfer", "secondary-nvinference-engine0");
      if (sg && nvds_parse_gie(sg, argv[1], "secondary-gie0") == NVDS_YAML_PARSER_SUCCESS) {
        sgie0 = sg;
        g_object_set(G_OBJECT(sgie0), "batch-size", multiurisrcbin_max_bs, NULL);
  ...
  /* Secondary GIE 1 (e.g. LPRNet on LPDNet plate crops) */
  {
    NvDsGieType sgie1_type = NVDS_GIE_PLUGIN_INFER;
    if (nvds_parse_gie_type(&sgie1_type, argv[1], "secondary-gie1") == NVDS_YAML_PARSER_SUCCESS) {
      GstElement *sg = gst_element_factory_make("nvinfer", "secondary-nvinference-engine1");
      if (sg && nvds_parse_gie(sg, argv[1], "secondary-gie1") == NVDS_YAML_PARSER_SUCCESS) {
        sgie1 = sg;
        g_object_set(G_OBJECT(sgie1), "batch-size", multiurisrcbin_max_bs, NULL);
