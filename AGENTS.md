...
- **Eventos por camión**: `_find_or_create_event` agrupa por `(source_id, truck_id)` — un evento por pasada física de camión. Merge cross-cámara por **solapamiento temporal** (ventana 12s, solo eventos de la OTRA cámara): la zona es de una sola vía, dos camiones no pueden estar pasando a la vez. El veto por color HSV se eliminó del merge proactivo (las cámaras exponen/WB el mismo contenedor muy distinto — dh~0.44). `_try_merge_event` (al finalizar) fusiona por solape de **ventanas de actividad** [primera/última detección] — no por `timestamp_end` (incluye cola de timeout ~15s que encadenaba camiones consecutivos). Color HSV solo sobrevive como veto para eventos consecutivos NO solapados.
