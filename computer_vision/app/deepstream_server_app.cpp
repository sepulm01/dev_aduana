...
static gboolean HEADLESS = FALSE;
...
main (int argc, char *argv[])
{
  ...
  HEADLESS = g_getenv ("HEADLESS") &&
      !g_strcmp0 (g_getenv ("HEADLESS"), "1");

  gchar *nvdspreprocess_config_file = (gchar *) "/opt/computer_vision/models/peoplenet/config_preprocess.txt";
  ...
  /* Create nvtracker for object tracking between frames */
  ...
  if (PERF_MODE) {
    ...
    if(prop.integrated) {
      ...
      if (!enc_enable) {
        if (HEADLESS) {
          g_print ("HEADLESS mode: using fakesink\n");
          appctx.sink = gst_element_factory_make ("fakesink", "nvvideo-renderer");
        } else {
#ifdef __aarch64__
  ...
  g_object_set (G_OBJECT (appctx.pgie),
   "config-file-path", "../models/trafficcamnet/pgie_config.yml", NULL);
  ...
  if (PERF_MODE) {
   ...
   gst_bin_add_many (GST_BIN (appctx.pipeline), appctx.queue1, appctx.preprocess,
                     appctx.pgie, appctx.queue2, nvtracker, queue_t,
